-- =====================================================================
--  SS TUITIONS - COMPLETE DATABASE SETUP
--  Run this ONCE in the Supabase SQL Editor of your ss-tuitions project.
--
--  Creates: 49 tables, 30 enum types, 121 indexes, 7 integrity triggers,
--           reference data (roles/exams/subjects), and the owner account.
--
...
-- =====================================================================
--  INTEGRITY RULES (triggers and checks)
-- =====================================================================
...
-- =====================================================================
--  REFERENCE DATA AND OWNER ACCOUNT
-- =====================================================================

INSERT INTO roles (code, name, description) VALUES
  ('ADMIN',   'Administrator', 'Full access to every part of the platform'),
  ('TUTOR',   'Tutor',         'Teaches assigned batches; cannot administer'),
  ('PARENT',  'Parent',        'Views their own children''s progress'),
  ('STUDENT', 'Student',       'Attends classes and takes tests')
ON CONFLICT (code) DO NOTHING;

INSERT INTO exams (code, name, is_active) VALUES
  ('JEE_MAIN',     'JEE Main',                        true),
  ('JEE_ADVANCED', 'JEE Advanced',                    true),
  ('EAMCET',       'TG EAPCET / EAMCET',              true),
  ('IPE',          'Intermediate Public Examination', true),
  ('NEET',         'NEET',                            true),
  ('SAT',          'SAT',                             true)
ON CONFLICT (code) DO NOTHING;
...
-- Owner account. Only the Argon2id hash is stored; the password itself
-- was generated outside the database and never written here.
INSERT INTO users (email, password_hash, full_name, status, is_superadmin, failed_login_count)
VALUES (
  'santhu77998@gmail.com',
  '$argon2id$v=19$m=65536,t=3,p=4$L6bWvVoscCT0QdbSF9eb6A$BVZyBQ36KgxMph3oXQFiG+KJq6E6+JdZXw0zGAKM5Lc',
  'Santhosh', 'active', true, 0
) ON CONFLICT DO NOTHING;

INSERT INTO user_roles (user_id, role_id)
SELECT u.id, r.id FROM users u CROSS JOIN roles r
 WHERE u.email = 'santhu77998@gmail.com' AND r.code = 'ADMIN'
ON CONFLICT DO NOTHING;

-- Records that Alembic revision 0001 is applied, so future migrations chain.
...
-- =====================================================================
--  VERIFICATION - the last statement returns a summary row.
--  Expect: 49 tables, 30 enums, 4 roles, 6 exams, 4 subjects, 1 owner.
-- =====================================================================
SELECT
  (SELECT COUNT(*) FROM information_schema.tables
     WHERE table_schema='public' AND table_type='BASE TABLE'
       AND table_name <> 'alembic_version')                    AS tables,
  (SELECT COUNT(DISTINCT t.typname) FROM pg_type t
     JOIN pg_enum e ON t.oid=e.enumtypid
     JOIN pg_namespace n ON n.oid=t.typnamespace
     WHERE n.nspname='public')                                 AS enum_types,
  (SELECT COUNT(*) FROM roles)    AS roles,
  (SELECT COUNT(*) FROM exams)    AS exams,
  (SELECT COUNT(*) FROM subjects) AS subjects,
  (SELECT COUNT(*) FROM users WHERE is_superadmin) AS owner_account;
